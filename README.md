...
This is an internal tool for a hypothetical bikeshare company. The application would be utilized by an employee who manages the bike stock at bike stations throughout particular region. A live feed of current bike locations is provided via Leaflet map, which informs the employees decision to transfer existing bikes or deploy new bikes. The company's utilization of this application will prevent missed rental opportunities due to insufficient stock at a given locatioin. All data is stored and updated in a NoSQL database. CouchDB and R Shiny server are hosted in an AWS EC2 instance. This application features strong use of CouchDB, R Shiny, Leaflet, and AWS.
